...
export function SebastianWorkspace({ userId }: SebastianWorkspaceProps) {
  ...
  const draftOutputs = useQuery(api.contentOutputGroups.list, { status: "draft", limit: 10 }) || [];
  ...
  const reviewCount = proposedLearnings.length + priorityIdeas.length + newIdeas.length + draftOutputs.length;
