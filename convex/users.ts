...
export const getUserByClerkId = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Prefer real users-table ids and newest record when duplicates exist.
    const validUsers = users.filter((u) => String(u._id).startsWith("kx"));
    const candidates = validUsers.length > 0 ? validUsers : users;
