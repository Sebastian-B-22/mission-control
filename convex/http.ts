...
const PROGRAM_CONFIGS: Record<string, {
  programType: "camp" | "spring" | "pdp" | "trial";
  region?: "agoura" | "pali";
  price: number;
  displayName: string;
  location: string;
  seasonDates: string;
  kitTags: string[];
  openPhoneLineId?: string;
  confirmationKind?: "camp" | "pdp";
}> = {
  "summer-camps-2026": {
    programType: "camp",
    region: "agoura",
    price: 65,
    displayName: "Agoura/Westlake AYSO Summer Soccer Camp",
    location: "Brookside Elementary",
    seasonDates: "June 22-July 31, 2026",
    kitTags: ["Agoura Summer Camp 2026"],
    openPhoneLineId: "PN2La7sbgD",
    confirmationKind: "camp",
  },
  "pali-camps-2026": {
    programType: "camp",
    region: "pali",
    price: 75,
    displayName: "Pali AYSO Summer Soccer Camp",
    location: "Palisades High School",
    seasonDates: "July 27-August 14, 2026",
    kitTags: ["Pali Summer Camp 2026"],
    openPhoneLineId: "PN8D2nxcA2",
    confirmationKind: "camp",
  },
  "pdp-fall-agoura-2026": {
    ...
    region: "agoura",
    price: 249,
    displayName: "Agoura/Westlake AYSO Fall Player Development Program",
    location: "Brookside Elementary",
    seasonDates: "Fridays Aug 21-Oct 30, 2026; no session Labor Day weekend",
    kitTags: ["Fall PDP Agoura 2026"],
    openPhoneLineId: "PN2La7sbgD",
    confirmationKind: "pdp",
  },
};
...
async function tagConvertKit(email: string, firstName: string, weekNumbers: string[], programTags?: string[]) {
  const tags = programTags?.length
    ? programTags
    : ["Summer Camp 2026", ...weekNumbers.map((n) => `Camp-Week-${n}`)];
  try {
    // Tag subscriber
...
function formatDivision(value?: string) {
  if (!value) return "";
  return value.toUpperCase().replace("U", "U");
}

function getPdpTime(division?: string) {
  if (division === "u7" || division === "u8") return "4-5 PM";
  if (division === "u10") return "5-6 PM";
  if (division === "u12" || division === "u14") return "6-7 PM";
  return "your division time";
}

function buildPdpConfirmationSMS(args: {
  childNames: string[];
  children: Array<{ firstName: string; division?: string; ageGroup?: string }>;
  pricing: { total: number };
  program: { displayName: string; location: string; seasonDates: string };
}) {
  const names = args.childNames.join(" & ");
  const divisionLines = args.children
    .map((child) => {
      const division = child.division || child.ageGroup;
      return child.firstName + ": " + formatDivision(division) + " Fridays " + getPdpTime(division);
    })
    .join("\n");

  let message = "CONFIRMED! " + names + " " + (args.childNames.length > 1 ? "are" : "is") + " registered for " + args.program.displayName + ".\n\n";
  message += "Practices: " + args.program.seasonDates + "\n";
  message += divisionLines + "\n\n";
  message += "Scrimmage games: Sundays Aug 30, Sep 27, Oct 18 (makeup Oct 25)\n";
  message += "Location: " + args.program.location + "\n";
  message += "Total Paid: $" + args.pricing.total + "\n\n";
  message += "Questions? Just reply to this text.\n\n-Coach Corinne";
  return message;
}

async function sendOpenPhoneMessage(phone: string, message: string, fromLineId = OPENPHONE_AGOURA_LINE) {
  try {
    ...
    if (!res.ok) {
      console.error("[OpenPhone] Message send failed:", res.status, await res.text());
    }
  } catch (e) {
    console.error("[OpenPhone] Message send error:", e);
  }
}

async function upsertOpenPhoneRegistrationContact(args: {
  familyId?: string;
  parent: { firstName: string; lastName: string; email: string; phone: string };
  children: Array<{ firstName: string; lastName: string; division?: string; ageGroup?: string }>;
  programSlug: string;
  programLabel: string;
}) {
  try {
    const childSummary = args.children.map((c) => c.firstName).join("/");
    const externalId = args.familyId
      ? "mc-family:" + args.familyId
      : "aspire-registration:" + args.parent.email.toLowerCase().trim();
    const listUrl = new URL("https://api.openphone.com/v1/contacts");
    listUrl.searchParams.set("maxResults", "1");
    listUrl.searchParams.append("externalIds", externalId);
    listUrl.searchParams.append("sources", "aspire-registration");

    const existing = await fetch(listUrl, { headers: { Authorization: OPENPHONE_API_KEY } });
    if (existing.ok) {
    ...
    const res = await fetch("https://api.openphone.com/v1/contacts", {
      ...
      body: JSON.stringify({
        defaultFields: {
          firstName: args.parent.firstName + (childSummary ? " (" + childSummary + ")" : ""),
          lastName: args.parent.lastName || null,
          company: "Aspire Soccer Coaching",
          role: "Parent - " + args.programLabel,
          emails: [{ name: "email", value: args.parent.email }],
          phoneNumbers: [{ name: childSummary ? "Parent of " + childSummary : "mobile", value: normalizePhone(args.parent.phone) }],
        },
        source: "aspire-registration",
        externalId,
      }),
    });

    if (!res.ok) {
      console.error("[OpenPhone] Contact upsert failed:", res.status, await res.text());
    }
  } catch (e) {
    console.error("[OpenPhone] Contact upsert error:", e);
  }
}
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    const { parent, children, emergencyContact, waiverAccepted, promoCode, season, programSlug, programType } = body as {
      programSlug?: string;
      programType?: string;
      parent: { firstName: string; lastName: string; email: string; phone: string };
      children: Array<{ firstName: string; lastName: string; age?: number; birthDate?: string; birthYear?: string | number; gender?: string; allergies?: string; practiceDay?: string; division?: string; ageGroup?: string; sessions: Record<string, { type: string; selectedDays?: string[] }> }>;
      emergencyContact: { name: string; phone: string };
      waiverAccepted: boolean;
      promoCode?: string;
      season?: string;
    };
    const effectiveProgramSlug = programSlug || season || "summer-2026";
    const programConfig = PROGRAM_CONFIGS[effectiveProgramSlug];
    const effectiveProgramType = programConfig?.programType || programType || "camp";
    ...
    // Server-side pricing
    let subtotal = 0;
    if (programConfig?.price && (effectiveProgramType === "pdp" || effectiveProgramType === "spring")) {
      subtotal = children.length * programConfig.price;
    } else {
    ...
    const accountCreditApplied = Math.min(Math.max(0, creditBalance.balance || 0), promoTotal);
    const total = Math.max(0, promoTotal - accountCreditApplied);
    console.log("[register] Pricing:", { subtotal, discount, accountCreditApplied, total });
    ...
    await ctx.runMutation(api.camp.createRegistration, {
      ...
      pricing: { subtotal, discount, accountCreditApplied, total },
  ...
  handler: httpAction(async (ctx, req) => {
    try {
      const body = await req.json() as {
        ...
        programType?: string;
        parent: { firstName: string; lastName: string; email: string; phone: string };
        children: Array<{ firstName: string; lastName: string; birthDate?: string; birthYear?: string | number; gender?: string; allergies?: string; practiceDay?: string; division?: string; ageGroup?: string; sessions: Record<string, unknown> }>;
        emergencyContact: { name: string; phone: string };
        waiverAccepted: boolean;
        promoCode?: string;
        pricing: { subtotal: number; discount: number; total: number };
      ...
      await ctx.runMutation(api.camp.markFreeRegistration, {
        registrationId: freePaymentIntentId,
        promoCode: promoCode || undefined,
      });
      ...
      await tagConvertKit(parent.email, parent.firstName, uniqueWeeks, programConfig?.kitTags);
      
      // Upsert family in CRM and Quo/OpenPhone contact
      let familyId: string | undefined;
      try {
        familyId = await ctx.runMutation(api.families.upsertFamily, {
      ...
      await upsertOpenPhoneRegistrationContact({
        familyId,
        parent,
        children,
        programSlug: effectiveProgramSlug,
        programLabel: programConfig?.displayName || "Aspire registration",
      });

      // Send SMS confirmation
      if (programConfig?.confirmationKind === "pdp") {
        const message = buildPdpConfirmationSMS({
          childNames,
          children,
          pricing,
          program: programConfig,
        });
        await sendOpenPhoneMessage(parent.phone, message, programConfig.openPhoneLineId);
      } else {
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    if (event.type === "payment_intent.succeeded") {
      ...
      if (reg) {
        const programSlug = reg.programSlug || reg.season || "summer-2026";
        const programConfig = PROGRAM_CONFIGS[programSlug];
        ...
        await tagConvertKit(reg.parent.email, reg.parent.firstName, uniqueWeeks, programConfig?.kitTags);
        
        // 2. Sync Quo/OpenPhone contact, then send confirmation via the correct line
        await upsertOpenPhoneRegistrationContact({
        ...
        if (programConfig?.confirmationKind === "pdp") {
          ...
          await sendOpenPhoneMessage(reg.parent.phone, message, programConfig.openPhoneLineId);
        } else {
...
// CORS preflight for trial-day routes
["/camp/trial-day/admin/registrations", "/camp/trial-day/admin/update"].forEach((p) => {
  http.route({
    path: p, method: "OPTIONS",
    handler: httpAction(async () => new Response(null, { status: 204, headers: campCors() })),
  });
});

// GET /camp/trial-day/admin/registrations - List all trial day registrations
http.route({
  path: "/camp/trial-day/admin/registrations", method: "GET",
  handler: httpAction(async (ctx, req) => {
    const pw = req.headers.get("X-Admin-Password");
    if (pw !== "Aspire2026!") return campError("Unauthorized", 401);
    
    const url = new URL(req.url);
    const eventDate = url.searchParams.get("eventDate") || undefined;
    const session = url.searchParams.get("session") || undefined;
    
    // Query campTrialDayRegistrations table directly
    let registrations = await ctx.runQuery(internal.trialDay.listRegistrations, {});
    
    // Filter by eventDate if provided
    if (eventDate) {
      registrations = registrations.filter((r: any) => r.eventDate === eventDate);
    }
    
    // Filter by session if provided  
    if (session) {
      registrations = registrations.filter((r: any) => r.session === session);
    }
    
    // Group by session for convenience
    const bySession: Record<string, typeof registrations> = {};
    for (const reg of registrations) {
      const s = reg.session || "Unknown";
      if (!bySession[s]) bySession[s] = [];
      bySession[s].push(reg);
    }
    
    return campJson({
      total: registrations.length,
      bySession: Object.entries(bySession).map(([sess, regs]) => ({
        session: sess,
        count: regs.length,
        registrations: regs,
      })),
...
// POST /camp/trial-day/admin/update - Update a registration (change session, cancel, etc)
http.route({
  path: "/camp/trial-day/admin/update", method: "POST",
  handler: httpAction(async (ctx, req) => {
    const pw = req.headers.get("X-Admin-Password");
    if (pw !== "Aspire2026!") return campError("Unauthorized", 401);
    ...
    const { registrationId, session, status } = body as {
      registrationId: string;
      session?: string;
      status?: string;
    };
    
    if (!registrationId) return campError("registrationId required");
    
    try {
      const result = await ctx.runMutation(internal.trialDay.updateRegistration, {
        registrationId,
        session,
        status,
      });
      ...
      return campError(e instanceof Error ? e.message : "Update failed", 400);
    }
  }),
});
